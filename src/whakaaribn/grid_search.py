...
def ap(estimator, X, y, w=1):
    ...
    score = average_precision_score(
        y, prob_e[:, 1], sample_weight=weights, average="weighted"
    )
    return score
...
def aic(estimator, X, y):
    ...
    """
    dof = np.sum(2 * estimator["clf"].nstates ** np.arange(1, 6))
    prob_e = estimator.predict_proba(X)
...
def my_auc(estimator, X, y, w=1):
    prob_e = estimator.predict_proba(X)
    weights = np.where(y == 1, w, 1)
    score = roc_auc_score(y, prob_e[:, 1], sample_weight=weights, average="weighted")
    return score


def my_roc_auc(estimator, X, y, eruptions, use_pew=False):
    prob_e = estimator.predict_proba(X)
    thresholds = np.linspace(0.01, 0.99, 100)[::-1]
    mdl = pd.DataFrame({"prob": prob_e[:, 1]}, index=X.index)
    mdl["eruptions"] = eruptions.reindex(mdl.index, fill_value=0)["Activity_Scale"]
    assert mdl.shape[0] > 0
    if use_pew:
        func = partial(evaluate_threshold, pew=estimator["clf"].pew)
    else:
        func = partial(evaluate_threshold, pew=None)
...
def grid_search(
    ...
    if fout is not None and recompute is False:
        if os.path.exists(fout):
            ...
            search_results = pd.read_csv(
                fout, index_col=(0, 1, 2), converters={"params": eval}
            )
    ...
    search = GridSearchCV(
        estimator=pipe,
        param_grid=params_gcv,
        scoring={
            "average_precision": partial(ap, w=1),
            "aic": aic,
            "log_loss": my_log_loss,
            "roc_auc": partial(my_auc, w=1),
            "mod_roc_auc": partial(my_roc_auc, use_pew=True, eruptions=eruptions),
            "mod_roc_auc_no_pew": partial(
                my_roc_auc, use_pew=False, eruptions=eruptions
            ),
        },
        cv=cv,
        n_jobs=njobs,
    ...
    search.fit(X_train.ffill(), y_train)
...
def get_best_estimator(grid_search_results: str) -> tuple:
    ...
    search_results = pd.read_csv(grid_search_results, converters={"params": eval})
    idx_max = search_results["mean_test_mod_roc_auc_no_pew"].idxmax()
    best_estimator = search_results.iloc[idx_max].params
    return best_estimator["discretize__bins"], best_estimator["clf__pew"]
